  ...
  const loadCategories = async () => {
    try {
      ...
      for (const event of events.data.data) {
        ...
        const category = res.data.data.map((item: any) => ({
          ...item,
  ...
  return (
    <div className="cdx-root">
      ...
      <div className="cdx-header-row">
        <div>
          <p className="cdx-eyebrow">Panel Admin — Manajemen Tiket</p>
          <h1 className="cdx-title">Ticket Categories</h1>
          <p className="cdx-subtitle">Kelola seluruh kategori tiket per event</p>
        </div>

        <button
          onClick={() => router.push("/categories/create")}
          className="cdx-add-btn"
        >
          <Plus size={16} />
          Tambah Category
        </button>
      ...
      <div className="cdx-panel">
        <div className="cdx-search-bar">
          ...
          <input
            type="text"
            placeholder="Cari berdasarkan nama kategori atau event..."
            value={search}
            onChange={(e) => {
        ...
        <div className="cdx-table-wrap">
          <CategoryTable categories={currentData} onDelete={remove} />
        </div>
